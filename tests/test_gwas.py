...
def test_create(client):
    ...
    response = client.post("create", data={"title": "test title", "description": "test description"})
    assert response.headers["Location"] == "http://localhost/index"

def test_update(client):
    client.post("/auth/register", data={"email": "a@a.a", "password": "a", "password_check": "a"})
    client.post("/auth/login", data={"email": "a@a.a", "password": "a"})
    client.post("create", data={"title": "testtitle", "description": "test description"})
    assert client.get("update/testtitle").status_code == 200
    response = client.post("update/testtitle", data={"title": "testtitle", "description": "test description"})
    assert response.headers["Location"] == "http://localhost/index"
